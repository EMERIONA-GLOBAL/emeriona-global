...
import type { CatalogCategory } from "./catalog-taxonomy.js";
...
export interface ProductRepositoryPortV1 { findById(id: ProductId): Promise<Product | null>; findCategories(id: ProductId): Promise<readonly CatalogCategory[]>; findPartnerOffers(id: ProductId): Promise<readonly PartnerOffer[]>; save(entity: Product): Promise<Product>; }
export interface ServiceRepositoryPortV1 { findById(id: ServiceId): Promise<Service | null>; findCategories(id: ServiceId): Promise<readonly CatalogCategory[]>; findPartnerOffers(id: ServiceId): Promise<readonly PartnerOffer[]>; save(entity: Service): Promise<Service>; }
...
export interface PartnerCatalogRepositoryPortV1 { saveProduct(entity: PartnerProduct): Promise<PartnerProduct>; updateProduct(id: ProductId, patch: { ownerId?: string; name?: string; status?: PartnerProduct["status"] }, partnerId: PartnerId, tenantId: string): Promise<PartnerProduct>; saveService(entity: PartnerService): Promise<PartnerService>; updateService(id: ServiceId, patch: { ownerId?: string; name?: string; status?: PartnerService["status"] }, partnerId: PartnerId, tenantId: string): Promise<PartnerService>; }
...
export const DOMAIN_REPOSITORY_PORTS_VERSION = "1.6.0" as const;
