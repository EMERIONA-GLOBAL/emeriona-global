...
export const BILLING_USE_CASE_IDS = {
  ...
  settlementCreate: "billing.settlement.create",
} as const;
...
export interface InvoiceRecord {
  ...
  readonly paymentIntentId: PaymentId;
...
export function assertInvoiceAmountMatchesPayment(invoiceAmount: Money, paymentAmount: Money): void {
  ...
  if (invoiceAmount.currency !== paymentAmount.currency || invoiceAmount.amount !== paymentAmount.amount) {
    throw new Error("invoice amount does not match payment intent");
  }
}

export function assertSettlementAmounts(gross: Money, commission: Money, net: Money): void {
  if (![gross, commission, net].every((value) => Number.isFinite(value.amount) && value.amount >= 0)) {
    throw new Error("settlement amounts must be finite and non-negative");
  }
  if (![gross.currency, commission.currency, net.currency].every((currency) => /^[A-Z]{3}$/.test(currency))) {
    throw new Error("settlement currency must be a 3-letter uppercase code");
  }
  ...
  if (gross.amount !== commission.amount + net.amount) throw new Error("settlement amounts do not balance");
}

export function assertSettlementOrderOwnership(orderId: OrderId, settlementOrderId: OrderId): void {
  if (orderId !== settlementOrderId) throw new Error("settlement order mismatch");
}
