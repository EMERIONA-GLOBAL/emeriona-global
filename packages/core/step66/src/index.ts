...
export function validateMetadata(metadata?: Readonly<Record<string, unknown>>): Result<void> {
  if (!metadata) return {ok:true,value:undefined};
  for (const key of Object.keys(metadata)) if (SENSITIVE.test(key)) return {ok:false,error:{code:"CORE_SENSITIVE_METADATA",message:`Sensitive metadata key rejected: ${key}`,category:"VALIDATION"}};
...
export function validateContext(context:RequestContext): Result<RequestContext> {
  if(!context.requestId || !context.tenantId) return fail({code:"CORE_CONTEXT_INVALID",message:"requestId and tenantId are required",category:"VALIDATION"});
  const meta=validateMetadata(context.metadata); return meta.ok ? ok(context) : meta;
}
export const STEP_66 = { name:"Shared Core & Contracts Integration Foundation", version:"1.0.0", status:"FOUNDATION", flow:"Identity → Context → Result/Error → Metadata → Versioning → Domain Events → Contracts → Validation", ownership:["shared identifiers","request context","result/error model","metadata conventions","semantic versioning","domain event envelopes","contract validation boundaries","shared validation primitives"], providerNeutral:true } as const;
