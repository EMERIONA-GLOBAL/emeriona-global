...
export interface OperationalCommercePort {
  ...
  checkout(input: CheckoutInput & { orderId: string; orderItemIds: readonly string[]; tenantId: string }): Promise<CheckoutResult>;
}
...
export class CheckoutHandler implements UseCaseHandler<CheckoutInput, CheckoutResult> {
  constructor(private readonly commerce: OperationalCommercePort, private readonly id: () => string, private readonly orderItemIds: () => string) {}
  async handle(request: UseCaseRequest<CheckoutInput>): Promise<UseCaseResponse<CheckoutResult>> {
    const result = await this.commerce.checkout({ cartId: required(request.input.cartId, "cartId"), orderId: this.id(), orderItemIds: [this.orderItemIds()], tenantId: request.context.tenantId });
    return { useCaseId: request.useCaseId, correlationId: request.context.correlationId, output: result };
  }
}

export const OPERATIONAL_COMMERCE_VERSION = "1.0.0" as const;
