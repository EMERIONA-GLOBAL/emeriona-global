...
export class DefaultUseCaseRuntime implements UseCaseRuntime{
 ...
 async execute<T,R>(handler:UseCaseHandler<T,R>,request:UseCaseRequest<T>):Promise<UseCaseResponse<R>>{
  ...
  const startedAt=Date.now();try{await recordAudit(this.dependencies.audit,request,"STARTED");const executeHandler=()=>handler.handle(request);const response=this.dependencies.transaction?await this.dependencies.transaction.run(executeHandler):await executeHandler();if(idempotency&&request.idempotencyKey)await idempotency.storeResult(request.idempotencyKey,idempotencyContext,response);await recordAudit(this.dependencies.audit,request,"SUCCEEDED");await this.dependencies.telemetry?.record({useCaseId:request.useCaseId,correlationId:request.context.correlationId,durationMs:Date.now()-startedAt,outcome:"SUCCEEDED"});return response;}catch(error){if(idempotency&&request.idempotencyKey){try{await idempotency.release(request.idempotencyKey,idempotencyContext);}catch{}}try{await recordAudit(this.dependencies.audit,request,"FAILED");await this.dependencies.telemetry?.record({useCaseId:request.useCaseId,correlationId:request.context.correlationId,durationMs:Date.now()-startedAt,outcome:"FAILED"});}catch{}throw new Error(errorMessage(error));}
 }
}
export const APPLICATION_RUNTIME_VERSION="1.2.0" as const;
