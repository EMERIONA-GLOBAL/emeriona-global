...
export const API_ROUTES: ReadonlyArray<ApiRouteDefinition> = [
  { method: 'GET', path: '/api/health', version: API_VERSION, kind: 'HEALTH' },
  ...USE_CASE_ROUTES,
];

export function resolveApiRoute(path: string, method: string): ApiRouteMatch | null {
  const normalizedMethod = method.toUpperCase();
  const route = API_ROUTES.find((candidate) => candidate.path === path && candidate.method === normalizedMethod);
  if (!route) return null;
  return { route, path, method: normalizedMethod as ApiMethod };
}

export function hasApiPath(path: string): boolean {
  return API_ROUTES.some((route) => route.path === path);
}

export function createApiError(
  code: ApiErrorCode,
  message: string,
  context?: Pick<ApiErrorBody, 'requestId' | 'correlationId'>,
): ApiErrorEnvelope {
