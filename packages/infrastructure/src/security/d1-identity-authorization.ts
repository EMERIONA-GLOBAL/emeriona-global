import type { IdentityAuthorizationPort, AuthIdentity, AuthPrincipal, AuthSession, TenantId } from "../../../application/src/identity-authorization.js";
...
export class D1IdentityAuthorizationAdapter implements IdentityAuthorizationPort {
  ...
  async findIdentityByLogin(tenantId: TenantId, loginNormalized: string): Promise<AuthIdentity | undefined> {
    const result = await this.db.prepare("SELECT id,tenant_id AS tenantId,principal_id AS principalId,login,login_normalized AS loginNormalized,status FROM auth_identities WHERE tenant_id=? AND login_normalized=? LIMIT 1").bind(tenantId, loginNormalized).all<AuthIdentity>();
    return result.results[0];
  }

  async findPrincipal(tenantId: TenantId, principalId: string): Promise<AuthPrincipal | undefined> {
    const result = await this.db.prepare("SELECT id,tenant_id AS tenantId,principal_type AS principalType,subject_id AS subjectId,status FROM auth_principals WHERE tenant_id=? AND id=? LIMIT 1").bind(tenantId, principalId).all<AuthPrincipal>();
    return result.results[0];
  }

  async validateSession(tenantId: TenantId, tokenHash: string, nowIso: string): Promise<AuthSession | undefined> {
    const result = await this.db.prepare("SELECT id,tenant_id AS tenantId,identity_id AS identityId,token_hash AS tokenHash,status,expires_at AS expiresAt FROM auth_sessions WHERE tenant_id=? AND token_hash=? AND status='ACTIVE' AND expires_at>? LIMIT 1").bind(tenantId, tokenHash, nowIso).all<AuthSession>();
    return result.results[0];
  }
