import type { CatalogId } from "../../../domains/src/catalog.js";
...
export class D1CatalogPublicationAdapter implements CatalogPublicationPort {
  ...
  async publishCatalog(catalogId: CatalogId): Promise<PublishCatalogOutput> {
    const catalog = await one<{ id: CatalogId; ownerId: string; name: string; status: "DRAFT" | "PUBLISHED" | "ARCHIVED" }>(
      this.db,
    ...
    return { catalog, publishedAt: new Date().toISOString() };
  }
...
export const C9_CATALOG_PUBLICATION_INFRASTRUCTURE_VERSION = "1.0.0" as const;
