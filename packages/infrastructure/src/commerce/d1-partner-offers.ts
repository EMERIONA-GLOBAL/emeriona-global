...
export class D1PartnerOffersAdapter implements PartnerOfferRepositoryPortV1 {
  ...
  async update(id:PartnerOfferId,patch:{name?:string;status?:PartnerOffer["status"]},tenantId:string):Promise<PartnerOffer>{
    const current=await this.db.prepare("SELECT o.id,o.partner_id AS partnerId,o.name,o.status,opl.product_id AS productId,osl.service_id AS serviceId FROM offers o LEFT JOIN offer_product_links opl ON opl.offer_id=o.id AND opl.tenant_id=o.tenant_id LEFT JOIN offer_service_links osl ON osl.offer_id=o.id AND osl.tenant_id=o.tenant_id WHERE o.id=? AND o.tenant_id=?").bind(id,tenantId).all<{id:string;partnerId:string;name:string;status:PartnerOffer["status"]}>();
