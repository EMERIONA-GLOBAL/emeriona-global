...
import { CreateCartHandler, CreateCustomerHandler, CreateOrderHandler, CreatePartnerHandler, CreateProductHandler, CreateServiceHandler, AddCartItemHandler, CheckoutHandler, DefaultUseCaseRuntime } from "../../application/src/index.js";
...
import { InMemoryIdempotencyAdapter, NoopAuditAdapter, NoopTelemetryAdapter, PolicyAuthorizationAdapter } from "./adapters/application-ports.js";
...
export function createFoundationRuntime(db:D1DatabaseLike,options:FoundationCompositionOptions):FoundationRuntime{
 ...
 const operationalCommerce=new D1OperationalCommerceAdapter(db);
 ...
 const runtime=new DefaultUseCaseRuntime({authorization,idempotency:new InMemoryIdempotencyAdapter(),audit:new NoopAuditAdapter(),telemetry:new NoopTelemetryAdapter()});
 const handlers=new Map<string,UseCaseHandler<unknown,unknown>>();
 const register=<I,O>(id:UseCaseId,handler:UseCaseHandler<I,O>)=>{if(handlers.has(id))throw new Error(`Use case already registered: ${id}`);handlers.set(id,handler as UseCaseHandler<unknown,unknown>);};
 ...
 register("cart.item.add" as UseCaseId,new AddCartItemHandler(operationalCommerce,()=>randomId("cit")));
 register("checkout.execute" as UseCaseId,new CheckoutHandler(operationalCommerce,()=>randomId("ord")));
