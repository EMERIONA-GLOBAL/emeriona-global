...
function errorResponse(
  code: Parameters<typeof createApiError>[0],
  message: string,
  status: number,
  context?: { requestId?: string; correlationId?: string },
): Response {
  return json(createApiError(code, message, context), status);
}
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const route = resolveApiRoute(path, request.method);

    if (!path.startsWith("/api/")) return env.ASSETS.fetch(request);

    if (path === "/api/health") {
      if (!route) return errorResponse("method_not_allowed", "Method not allowed", 405);
      return json({
    ...
    if (!route || route.route.kind !== "USE_CASE" || !route.route.useCaseId) {
      return errorResponse("method_not_allowed", "Method not allowed", 405);
    }

    const useCaseId = route.route.useCaseId as UseCaseId;
    let runtimeContext;
    try {
      runtimeContext = createRuntimeHttpContext({
        request,
        service: "emeriona-global-api",
        environment: "PRODUCTION",
        version: RUNTIME_HTTP_VERSION,
      });
      validateRuntimeHttpPolicy(request);
    } catch (error) {
    ...
    if (!actorId) {
      return errorResponse(
        "tenant_and_actor_context_required",
        "Tenant and actor context are required",
        400,
        requestContext,
      );
    }
    ...
    if (!validCurrency(currency)) {
      return errorResponse("valid_currency_required", "A valid ISO 4217 currency code is required", 400, requestContext);
    }
    ...
    try {
      ...
      return errorResponse("invalid_json", "Request body must be valid JSON", 400, requestContext);
    }
    ...
    const envelope: UseCaseRequest<unknown> = {
      useCaseId,
      context: {
        tenantId: runtimeContext.tenantId as UseCaseRequest<unknown>["context"]["tenantId"],
        correlationId: runtimeContext.correlationId as UseCaseRequest<unknown>["context"]["correlationId"],
        actorId,
        requestId: runtimeContext.requestId,
    ...
    try {
      const result = await foundation.execute<unknown, unknown>(envelope);
      return json({
        data: result.output,
        meta: {
      ...
      return errorResponse(
        "use_case_failed",
        error instanceof Error ? error.message : "Use case execution failed",
        400,
        requestContext,
      );
    }
