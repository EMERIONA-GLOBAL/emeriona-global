...
curl -fsS "\${COMMON[@]}" -H "idempotency-key: \${UPDATE_KEY}" -X POST "\$BASE/api/v1/carts/update" -d "{\"cartId\":\"\${C}\",\"customerId\":\"\${C2}\"}" > update.json
...
REPLAY_STATUS="$(curl -sS "\${COMMON[@]}" -H "idempotency-key: \${UPDATE_KEY}" -X POST "\$BASE/api/v1/carts/update" -d "{\"cartId\":\"\${C}\",\"customerId\":\"\${C2}\"}" -o replay.json -w '%{http_code}')"
...
CROSS_STATUS="$(curl -sS "\${COMMON2[@]}" -H "idempotency-key: cart-update-cross-\${GITHUB_RUN_ID:-manual}" -X POST "\$BASE/api/v1/carts/update" -d "{\"cartId\":\"\${C}\",\"customerId\":\"\${C3}\"}" -o cross.json -w '%{http_code}')"
test "\$CROSS_STATUS" = '400'
EMPTY_STATUS="$(curl -sS "\${COMMON[@]}" -H "idempotency-key: cart-update-empty-\${GITHUB_RUN_ID:-manual}" -X POST "\$BASE/api/v1/carts/update" -d "{\"cartId\":\"\${C}\"}" -o empty.json -w '%{http_code}')"
