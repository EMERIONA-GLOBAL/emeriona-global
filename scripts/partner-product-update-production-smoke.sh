...
jq -e --arg id "$P" --arg tenant "$T" --arg partner "$PTR" '.[0].results[0].id == $id and .[0].results[0].tenant_id == $tenant and .[0].results[0].partner_id == $partner and .[0].results[0].name == "Updated Partner Product" and .[0].results[0].status == "PUBLISHED"' persistence.json
