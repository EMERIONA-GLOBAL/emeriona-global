...
curl -fsS "${COMMON[@]}" -H "idempotency-key: c10-offer-update-${GITHUB_RUN_ID}" -X POST "$BASE/api/v1/partners/offers/update" -d "{\"offerId\":\"${OFFER_ID}\",\"name\":\"C10 Active Offer\",\"status\":\"ACTIVE\"}" > update.json
jq -e --arg id "$OFFER_ID" '.data.id == $id and .data.name == "C10 Active Offer" and .data.status == "ACTIVE"' update.json
curl -fsS "${COMMON[@]}" -H "idempotency-key: c10-offer-update-${GITHUB_RUN_ID}" -X POST "$BASE/api/v1/partners/offers/update" | jq -e --arg id "$OFFER_ID" '.data.id == $id and .data.status == "ACTIVE"'
