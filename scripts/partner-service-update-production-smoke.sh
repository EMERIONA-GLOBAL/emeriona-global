...
T="partner-service-update-smoke-${GITHUB_RUN_ID}"; T2="partner-service-update-isolation-${GITHUB_RUN_ID}"; S="partner-service-${GITHUB_RUN_ID}"; PTR="partner-service-owner-${GITHUB_RUN_ID}"; OWNER="partner-service-owner-${GITHUB_RUN_ID}"
cleanup() { npx wrangler d1 execute emeriona-global-db --remote --command="DELETE FROM services WHERE id='${S}'; DELETE FROM partners WHERE id='${PTR}'; DELETE FROM tenants WHERE id IN ('${T}','${T2}');" >/dev/null 2>&1 || true; }
trap cleanup EXIT
npx wrangler d1 execute emeriona-global-db --remote --command="INSERT INTO tenants (id,name,status) VALUES ('${T}','Partner Service Update Smoke ${GITHUB_RUN_ID}','ACTIVE'),('${T2}','Partner Service Update Isolation ${GITHUB_RUN_ID}','ACTIVE'); INSERT INTO partners (id,tenant_id,legal_name,status) VALUES ('${PTR}','${T}','Partner Service Update Smoke','VERIFIED'); INSERT INTO services (id,tenant_id,owner_id,partner_id,name,status) VALUES ('${S}','${T}','${OWNER}','${PTR}','Original Partner Service','DRAFT');"
