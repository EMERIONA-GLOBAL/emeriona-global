...
EVIDENCE="SELECT (SELECT count(*) FROM cart_items WHERE cart_id='${C}' AND product_id='${P}' AND quantity=2 AND unit_amount=25 AND currency='USD') AS item_ok,(SELECT count(*) FROM cart_items WHERE cart_id='${C}') AS item_count,(SELECT count(*) FROM idempotency_records WHERE tenant_id='${T}' AND idempotency_key='${KEY}' AND status='COMPLETED') AS idempotency_ok;"
