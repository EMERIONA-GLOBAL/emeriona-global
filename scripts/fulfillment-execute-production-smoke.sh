...
curl -fsS "${COMMON[@]}" -H "idempotency-key: fulfillment-payment-${RUN_ID}" -X POST "$BASE/api/v1/payment-intents" -d "{\"orderId\":\"${ORDER_ID}\",\"amount\":{\"amount\":90,\"currency\":\"USD\"}}" | tee /tmp/fulfillment-payment.json
PAYMENT_ID=$(jq -r '.data.id' /tmp/fulfillment-payment.json)
curl -fsS "${COMMON[@]}" -H "idempotency-key: fulfillment-authorize-${RUN_ID}" -X POST "$BASE/api/v1/payment-intents/${PAYMENT_ID}/authorize" -d '{}' >/tmp/fulfillment-authorize.json
curl -fsS "${COMMON[@]}" -H "idempotency-key: fulfillment-capture-${RUN_ID}" -X POST "$BASE/api/v1/payment-intents/${PAYMENT_ID}/capture" -d '{}' >/tmp/fulfillment-capture.json
...
test "$STATUS" = '400'
SQL="SELECT (SELECT count(*) FROM fulfillments WHERE id='${FULFILLMENT_ID}' AND tenant_id='${TENANT_ID}' AND order_id='${ORDER_ID}' AND status='PENDING') AS fulfillment_ok, (SELECT count(*) FROM fulfillments WHERE order_id='${ORDER_ID}' AND tenant_id='${OTHER_TENANT_ID}') AS cross_tenant_rows, (SELECT count(*) FROM fulfillment_events WHERE tenant_id='${TENANT_ID}' AND fulfillment_id='${FULFILLMENT_ID}' AND to_status='PENDING') AS event_ok, (SELECT count(*) FROM idempotency_records WHERE tenant_id='${TENANT_ID}' AND use_case_id='fulfillment.execute' AND idempotency_key='fulfillment-execute-${RUN_ID}' AND status='COMPLETED') AS idempotency_ok, (SELECT count(*) FROM audit_events WHERE tenant_id='${TENANT_ID}' AND use_case_id='fulfillment.execute' AND outcome='SUCCEEDED') AS audit_ok;"
